   define ciscoaci::setup_dhclient_file($interface_name, $opflex_uplink_iface) {
     ...
     #$macaddr = generate("/bin/cat", "/sys/class/net/$real_opflex_uplink_iface/address")
      ...
      if($::osfamily == 'Redhat') {
        $cmdstr = "/bin/bash -c '_xyz=`/bin/cat /sys/class/net/${opflex_uplink_iface}/address`; printf \"send dhcp-client-identifier 01:%s;\" \$_xyz > /etc/dhcp/dhclient-${interface_name}.conf' "
